...
num = read_in()
fileName = "C:/Users/user/Downloads/example" + num + ".png"

img = []
list = []
model = load_model('emnist_trained.h5')  # 훈련 모델 데려오기
img_color = cv2.imread(fileName, cv2.IMREAD_COLOR)  # 이미지 불러오기 (경로만 바꾸어주면 됨)
...
for i in range(2):
    ...
    #sorted_ctrs = sorted(contours, key=lambda ctr: cv2.boundingRect(contours)[0])
